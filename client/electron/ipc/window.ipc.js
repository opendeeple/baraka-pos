...
function writeSessions(data) {
    try {
        writeFileSync(getSessionsFile(), JSON.stringify(data));
    }
    catch { }
}
const miniappViews = new Map();
...
export function registerFullscreenIpc(getMainWindow) {
...
// ─────────────────────────────────────────────────────────────────────────────
export function registerWindowIpc(createCustomerWindow, getCustomerWindow) {
    const sessions = readSessions();
    ipcMain.handle('window:openCustomerDisplay', () => {
        if (!getCustomerWindow())
    ...
    ipcMain.handle('window:updateCustomerDisplay', (_event, data) => {
    ...
    ipcMain.on('webview:get-session', (event, origin) => {
        event.returnValue = sessions[origin] ?? null;
    });
    ipcMain.handle('webview:save-session', (_event, origin, data) => {
        sessions[origin] = data;
        writeSessions(sessions);
    });
    const openAppWindows = new Map();
    ipcMain.handle('window:openApp', (_event, { id, name, url }) => {
    ...
    ipcMain.handle('miniapp:open', (_event, { id, url }) => {
        const win = getMainWin();
        if (!win)
            return;
        ...
        if (!entry || entry.view.webContents.isDestroyed()) {
            ...
            view.webContents.on('did-start-loading', () => {
                if (!win.isDestroyed())
                    win.webContents.send('miniapp:state', { id, loading: true, error: undefined });
            });
            view.webContents.on('did-fail-load', (_e, errorCode, errorDescription) => {
                if (win.isDestroyed() || errorCode === -3)
                    return; // -3 = aborted navigation, ignore
            ...
            view.webContents.on('did-stop-loading', async () => {
                ...
                saveTimer = setTimeout(async () => {
                    try {
                        const ssJson = await view.webContents.executeJavaScript('JSON.stringify(Object.fromEntries(Object.entries(sessionStorage)))');
            ...
            view.webContents.on('page-title-updated', (_, title) => {
                if (!win.isDestroyed())
                    win.webContents.send('miniapp:state', { id, title });
            });
        ...
        try {
            entry.view.setBounds(calcBounds(win));
        }
        catch { }
        activeViewId = id;
    ...
    ipcMain.handle('miniapp:navigate', (_event, action) => {
        if (!activeViewId)
            return;
        const entry = miniappViews.get(activeViewId);
        if (!entry)
    ...
    ipcMain.handle('miniapp:destroy', (_event, id) => {
        const win = getMainWin();
    ...
    ipcMain.handle('miniapp:setBounds', (_event, bounds) => {
        if (!activeViewId)
            return;
        const entry = miniappViews.get(activeViewId);
        if (entry?.added) {
