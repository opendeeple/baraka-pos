...
function getReceiptLayoutConfig() {
    try {
        const rows = dbQuery(`SELECT meta_value FROM settings WHERE meta_key='receipt_layout' LIMIT 1`, []);
        if (rows.length > 0) {
            const saved = JSON.parse(rows[0].meta_value);
            return {
...
function unconfiguredReason(config) {
    if (config.type === 'windows')
        return config.name.trim() ? null : 'No printer selected';
...
function centerPad(s, width, fontScale = 1) {
...
function fakeBarcode(seed, width) {
    let n = 0;
...
function receiptHtml(doc, layout) {
    ...
    const scaleOf = (k) => els[k].fontPx / REFERENCE_FONT_PX;
    const styleOf = (k, extra = '') => `font-size:${els[k].fontPx}px;margin-left:${els[k].shiftPx}px;${extra}`;
    const div = (k, text, extraCss = '') => `<div style="${styleOf(k, extraCss)}">${esc(text)}</div>`;
    const lines = [];
    lines.push(div('storeName', centerPad(doc.storeName, W, scaleOf('storeName')), 'font-weight:700;letter-spacing:0.5px;'));
    if (doc.storeAddress)
        lines.push(div('storeInfo', centerPad(doc.storeAddress, W, scaleOf('storeInfo')), 'font-weight:700;'));
    if (doc.storePhone)
        lines.push(div('storeInfo', centerPad(doc.storePhone, W, scaleOf('storeInfo')), 'font-weight:700;'));
    if (doc.header)
        lines.push(div('storeInfo', centerPad(doc.header, W, scaleOf('storeInfo')), 'font-weight:700;'));
    lines.push(`<div class="divider"></div>`);
    lines.push(div('invoiceInfo', `Invoice: ${doc.invoiceNumber}`));
    if (doc.cashierName)
        lines.push(div('invoiceInfo', `Cashier: ${doc.cashierName}`));
    ...
    for (const c of doc.charges)
        lines.push(div('totals', padRow(c.name, c.amount.toLocaleString(), W)));
    if (doc.discount)
        lines.push(div('totals', padRow('DISCOUNT:', `-${doc.discount.toLocaleString()}`, W)));
    lines.push(div('totalRow', padRow('TOTAL AMOUNT:', doc.total.toLocaleString(), W), 'font-weight:700;'));
    for (const p of doc.payments)
        lines.push(div('totals', padRow(`${p.method.toUpperCase()}:`, p.amount.toLocaleString(), W)));
    if (doc.change > 0)
        lines.push(div('totals', padRow('CHANGE:', doc.change.toLocaleString(), W)));
    lines.push(`<div class="divider"></div>`);
    if (doc.footer)
        lines.push(div('footer', centerPad(doc.footer, W, scaleOf('footer')), 'font-weight:700;letter-spacing:0.5px;'));
...
function printOnWindowsPrinter(deviceName, doc, layout) {
        ...
        win.webContents.once('did-finish-load', () => {
            ...
            setTimeout(async () => {
                ...
                try {
                    ...
                    // content instead.
                    const scrollHeightPx = await win.webContents.executeJavaScript('document.body.scrollHeight');
...
export function registerPrinterIpc() {
    ipcMain.handle('printer:print', async (_event, receiptData) => {
        try {
            ...
            const { Printer, USB, Network } = await import('escpos');
    ...
    ipcMain.handle('printer:configure', (_event, config) => {
...
function getPrinterConfig() {
    try {
        const rows = dbQuery(`SELECT meta_value FROM settings WHERE meta_key='printer_config' LIMIT 1`, []);
        if (rows.length > 0)
